...
class AuthProfileUpdateRequested extends AuthEvent {
  ...
  final String school;
  final String grade;
  AuthProfileUpdateRequested({required this.name, required this.school, required this.grade});
}
    ...
    on<AuthProfileUpdateRequested>((event, emit) async {
      ...
      if (currentState is Authenticated) {
        ...
        try {
          ...
          emit(AuthError(e.toString()));
          emit(currentState); // Return to card
        }
      }
    });

    on<AuthAccountDeletionRequested>((event, emit) async {
      emit(AuthLoading());
      try {
